...
class Model(nn.Module):
    ...
    def val(self, validator=None, **kwargs):
        ...
        参数:
            validator (BaseValidator): 自定义验证器。
            **kwargs : 验证器接受的其他参数。查看文档中的'configuration'部分以获取所有可用选项
        """
        custom = {'rect': True}  # 方法默认值
        print("YES! You are in val function!")
        args = {**self.overrides, **custom, **kwargs, 'mode': 'val'}  # 最高优先级的参数在右边
    ...
    def train(self, trainer=None, **kwargs):
        ...
        if not args.get('resume'):  # manually set model only if not resuming
            ...
            # # 打印模型的类别数
            # print(f"Number of classes in model: {self.trainer.model}")
